...
<div class="flex items-center gap-3 mb-6 text-xs">
    ...
    <i class="fa-solid fa-chevron-right text-[10px] text-gray-300"></i>
    <span class="text-gray-600 font-bold">Edit Supplier</span>
</div>
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
        <div class="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
            ...
            <div>
                <h2 class="text-xl font-bold text-gray-800">Edit Data Supplier</h2>
                <p class="text-xs text-gray-400 mt-0.5">Perbarui informasi kontak dan alamat supplier.</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.suppliers.update', $supplier->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" required value="{{ old('name', $supplier->name) }}" placeholder="Contoh: PT. Kimia Farma Trading"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $supplier->phone) }}" placeholder="08123456789"
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Email Supplier</label>
                    <input type="email" name="email" value="{{ old('email', $supplier->email) }}" placeholder="supplier@apotek.com"
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="address" rows="3" placeholder="Jl. Industri No. 45, Jakarta"
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition">{{ old('address', $supplier->address) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="3" placeholder="Catatan mengenai jenis obat / sales person..."
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition">{{ old('notes', $supplier->notes) }}</textarea>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-3 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary/90 shadow-md shadow-primary/20 transition-all">
                    <i class="fa-solid fa-floppy-disk mr-2"></i> Perbarui Supplier
                </button>
            </div>
        </form>
    </div>
