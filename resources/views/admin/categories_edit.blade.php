...
<div class="flex items-center gap-3 mb-6 text-xs">
    ...
    <a href="{{ route('admin.categories') }}" class="text-gray-400 hover:text-primary transition">Kategori Obat</a>
    <i class="fa-solid fa-chevron-right text-[10px] text-gray-300"></i>
    <span class="text-gray-600 font-bold">Edit Kategori</span>
</div>
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-border-muted p-6 md:p-8">
        <div class="flex items-center gap-4 mb-6 pb-6 border-b border-border-muted">
            ...
            <h2 class="text-xl font-bold text-text-main">Edit Kategori</h2>
        </div>
        ...
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ $category->name }}" class="w-full px-4 py-3 border border-border-muted rounded-lg text-sm focus:border-primary focus:ring-4 focus:ring-primary-light outline-none transition" required>
            </div>
            ...
            <div>
                ...
                <div class="mb-4">
                    ...
                    <img src="{{ str_starts_with($category->image, '/') ? $category->image : '/' . $category->image }}" alt="{{ $category->name }}" class="w-24 h-24 object-cover rounded-lg border border-border-muted">
                </div>
                @endif
